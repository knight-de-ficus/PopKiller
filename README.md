...
- **系统信息**：显示 Windows 版本、版本号、注册用户与组织；
- **弹窗拦截**：点击进入规则管理页面；
- **设置**：点击进入设置页面。

鼠标悬停卡片时显示辉光效果，点击即可跳转。也可通过左侧导航栏或托盘菜单切换页面。
...
- 拦截到弹窗时，系统右下角弹出 Toast 通知；
- 通知显示被拦截的进程名与窗口标题；
- 通知不影响程序正常运行，可随时在设置中关闭；
- 关闭后仅写入日志，不再弹出系统通知。
...
在「拦截日志」页面可实时查看日志，支持搜索、过滤、手动刷新和清空。日志文件超过 1 MiB 时自动截断。
